...
contract CryptoStrippers{
    ...
    */
    function calculateTrade(uint256 n,uint256 c, uint256 m) public view returns(uint256){
//        return (PSN - m) / (PSNH + (((PSN - c) + (PSNH - n)) / n));


    return PSNH + (((PSN - c) + (PSNH - n)) / n);
    }

    function calculateTrade1(uint256 n,uint256 c, uint256 m) public view returns(uint256){
        return ((10000* m)/(5000+(((PSN* c)+(PSNH* n))/ n)));
    }
    ...
    function getMyCoins() public constant returns(uint256){
